...
static complex* _raw_radarfft(complex* data, unsigned N, bool is_forward) {
	...
	*b = cexp(e);
	for (unsigned q = 1; q < N - 1; q++) {
		complex e = { 0., -2 * PI * g_mod_minus_q * k / N };
		*(b + q) = cexp(e);
		// pow(g, -2) (mod N)
	...
	complex* fft_a = radix2fft(new_a, M);
	free(new_a);
	complex* fft_b = radix2fft(new_b, M);
	free(new_b);
	...
	complex* conv_ab = radix2ifft(fft_ab, M);
	free(fft_ab);
	...
	*(result + 1) = cadd(*data, cdiv(*conv_ab, cM));
	for (unsigned q = 1; q < N - 1; q++) {
		*(result + g_mod_minus_q) = cadd(*data, cdiv(*(conv_ab + q), cM));
		// pow(g, -2) (mod N)
		//      = { [pow(g, -1) (mod N)] * [pow(g, -1) (mod N)] } % N
		//		= [ pow(g, -1) * pow(g, -1) ] (mod N)
		g_mod_minus_q = (g_mod_minus_q * g_mod_minus_one) % N;
...
static complex* _raw_hybrid_radixfft(complex* data, unsigned N, bool is_forward) {
	...
	for (unsigned n1 = 0; n1 < N1; n1++) {
		...
		complex* fft_inner = _raw_hybrid_radixfft(inner, N2, is_forward);
		free(inner);
		for (unsigned n2 = 0; n2 < N2; n2++) {
	...
	for (unsigned n2 = 0; n2 < N2; n2++) {
		...
		complex* fft_outer = _raw_hybrid_radixfft(outer, N1, is_forward);
		free(outer);
		for (unsigned n1 = 0; n1 < N1; n1++) {
